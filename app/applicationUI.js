 ...
 *
 * applicationUI.js
 * Handles generation of main application UI components
 */
 ...
 */
function generateControlsHeaderHTML() {
    const html = `
      <div id="controlsHeader" >
        <div id="topBar" class="topBanner">
            <div>                
                <img id="brain" type="image" alt="BrainTool" src="../extension/images/BrainTool48.png"
                     height="40" width="40" >
	              <span id="searchCancelButton" class="searchButton" title="Exit Search">
                ...
                <span data-wenk="Filter TODOs" class="wenk--top filter-button"><img id="todoFilter" src="resources/star-transparent.svg"></span>
                <span data-wenk="Filter Matches" class="wenk--top filter-button"><img id="searchFilter" src="resources/filter.svg" style="display:none"></span>
	              <span class="searchButton" >
	                <span id="searchUpBtn" title="Previous Match">
                        <img src="resources/arrowUp.png" height="8" width="10"/>
                  </span>
	                <span id="searchDownBtn" title="Next Match">
        ...
        <div id="trialExpiredWarning">
          <img id="trialWarningClose" class="panelClose" type="image" alt="Close" title="Close"
          ...
          <div id="nagContent">
            ...
            <p id="nagText">Hello there! <br/>It looks like your 30 trial has expired and you haven't signed up as a Supporter. Please consider doing so to support the creation and ongoing evolution of BrainTool.<br/> <a href="#" id="trialWarningLink">Go to Settings for Supporter options.</a></p>
          </div>
        </div>
      </div>`;
 ...
 */
function attachControlsHeaderListeners() {
    // Search cancel button
    document.getElementById('searchCancelButton')?.addEventListener('mousedown', (e) => {
        searchButton(e, 'exit');
    });
    ...
    document.getElementById('searchFilter')?.addEventListener('mousedown', (e) => {
        filterSearch(e);
    });
    
    // Search up button
    document.getElementById('searchUpBtn')?.addEventListener('mousedown', (e) => {
        searchButton(e, 'up');
    });
    
    // Search down button
    document.getElementById('searchDownBtn')?.addEventListener('mousedown', (e) => {
        searchButton(e, 'down');
    });
    ...
    document.getElementById('newTopLevelTopic')?.addEventListener('click', () => {
        addNewTopLevelTopic();
    });
...
/**
 * Initialize the Controls Header - generate HTML and attach event listeners
 */
function initializeControlsHeader() {
    generateControlsHeaderHTML();
    attachControlsHeaderListeners();
    console.log('Controls header initialized with event listeners');
}

/**
 * Generate the Settings Panel HTML (without inline event handlers)
 */
      ...
      <div id="settingsSubscription">
        ...
        <div id="settingsSubscriptionAdd">
          ...
          <div id="settingsSubscriptionButtons" class="settingsInput">
            <button class="subButton" data-plan="Monthly">Month $1.33</button>
            <button class="subButton" data-plan="Annual">Year $10.66</button>
            <button class="subButton" data-plan="OTP">Lifetime $20.99</button>
          </div>
          ...
          <div class="settingsFootnote">
            ...
            <div class="settingsFootnote" id="importKey">Click here to manually enter a license code.</div>
          </div>
        </div>
        <div id="settingsSubscriptionStatus" style="display:none">
          ...
          <a id="portalLink" href="https://billing.stripe.com" title="Manage Subscription" target="_blank">Manage your subscription</a>
        </div>
      </div>
    </div>
    `;
...
function attachSettingsListeners() {
    ...
    const subButtons = document.querySelectorAll('#settingsSubscriptionButtons .subButton');
...
/**
 * Initialize Settings Panel by generating HTML and attaching event listeners
 */
function initializeSettings() {
    generateSettingsHTML();
    attachSettingsListeners();
    console.log('Settings panel initialized with event listeners');
}

/**
 * Generate the Actions Panel HTML (without inline event handlers)
 */
    ...
    <div class="settingsActions" id="actions">
        ...
        <div id="actionsImportExport">
            ...
            <div id="settingsDropdowns" class="settingsInput">
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown_content">
                        <div id="exportBookmarksItem">
                            <label class="button_text" for="export_bookmarks">Browser Bookmarks</label>
                            <input id="export_bookmarks" type="button" style="display:none">
                        </div>

                        <div style="border-top: solid 1px #ddd">
                            <a id="org_export" class="button_text" href="" download="BrainTool.org" style="text-align:left; text-decoration: none">Local Org-mode file</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div id="actionsSyncStatus" style="display:none">
          ...
          <div class="settingsInput">
            <button id="refresh" class="button_text">Refresh from File</button>
            <button id="stopSyncing" class="button_text">Stop Syncing</button>
          </div>
...
/**
 * Attach event listeners to Actions Panel elements
 */
function attachActionsListeners() {
    ...
    // Import bookmarks
    document.getElementById('importBookmarksItem')?.addEventListener('click', () => {
        importBookmarks();
    });
    
    // Import org file - clear value and trigger change
    document.getElementById('importOrgFileItem')?.addEventListener('click', () => {
        const orgUpload = document.getElementById('org_upload');
        if (orgUpload) {
            orgUpload.value = '';
        }
    });
    
    document.getElementById('org_upload')?.addEventListener('change', () => {
        importOrgFile();
    });
    
    // Import TabsOutliner - clear value and trigger change
    document.getElementById('importTabsOutlinerItem')?.addEventListener('click', () => {
        const toUpload = document.getElementById('to_upload');
        if (toUpload) {
            toUpload.value = '';
        }
    });
    
    document.getElementById('to_upload')?.addEventListener('change', () => {
        importTabsOutliner();
    });
    
    // Export bookmarks
    document.getElementById('exportBookmarksItem')?.addEventListener('click', () => {
        exportBookmarks();
    });
    
    // Export org file
    document.getElementById('org_export')?.addEventListener('click', () => {
        exportOrgFile();
    });
    ...
    // Stop syncing
    document.getElementById('stopSyncing')?.addEventListener('click', () => {
        stopSyncing();
    });
}

/**
 * Initialize Actions Panel by generating HTML and attaching event listeners
 */
function initializeActions() {
    generateActionsHTML();
    attachActionsListeners();
    console.log('Actions panel initialized with event listeners');
}

/**
 * Generate the Help Panel HTML (without inline event handlers)
 */
function generateHelpHTML() {
    ...
    <div class="settingsActions" id="help">
        ...
        <hr/>
        <div>
            <div class="settingsSubtitle helpAction" id="helpUserGuide">User Guide and General Help</div>
        </div>
        <hr/>
        <div>
            <div id="helpKeys" class="settingsSubtitle helpAction">
            ...
            <div id="keyCommands" style="display:none;">
                ...
                <tr>
                    <td></td>
                    <td class="keyCol">Ctrl+c/Cmd+c</td>
                    <td>Copy selection and children to clipboard</td>
                </tr>
        ...
        <hr/>
        <div>
            <div class="settingsSubtitle helpAction" id="helpFAQ">FAQs and How-tos</div>
        </div>
        <hr/>
        <div>
            <div class="settingsSubtitle helpAction" id="helpWebsite">BrainTool Website</div>
        </div>
    </div>
    `;
...
/**
 * Attach event listeners to Help Panel elements
 */
function attachHelpListeners() {
    ...
    // User Guide
    document.getElementById('helpUserGuide')?.addEventListener('click', () => {
        window.open('https://braintool.org/support/userGuide10.html');
    });
    ...
    // FAQs
    document.getElementById('helpFAQ')?.addEventListener('click', () => {
        window.open('https://braintool.org/support.html');
    });
    
    // Website
    document.getElementById('helpWebsite')?.addEventListener('click', () => {
        window.open('https://braintool.org');
    });
}

/**
 * Initialize Help Panel by generating HTML and attaching event listeners
 */
function initializeHelp() {
    generateHelpHTML();
    attachHelpListeners();
    console.log('Help panel initialized with event listeners');
}

/**
 * Generate the Footer HTML (without inline event handlers)
 */
...
/**
 * Initialize Footer by generating HTML and attaching event listeners
 */
